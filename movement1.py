...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    motorspeed=Kp* e_angle +Kd*(e_angle-e_angle_prev)
    e_angle_prev=e_angle

    rightMotorSpeed = 30 +motorspeed
    leftMotorSpeed = 30 - motorspeed 
    if (rightMotorSpeed>35):
	rightMotorSpeed=35
    elif (rightMotorSpeed<20):
	rightMotorSpeed=20
    if (leftMotorSpeed>35):
	leftMotorSpeed=35
    elif (leftMotorSpeed<20):
	leftMotorSpeed=20
    print"left motorspeed %d ---- right motorspeed %d ---- error angle %d" % (rightMotorSpeed,leftMotorSpeed,e_angle)
