...
def PID(e_angle_current,Kp,Kd,Ki):
        ...
        global error_prev
        global error_sum
        e_angle_target=e_angle_target+e_angle_current
        error=e_angle_target-e_angle_current
        error_sum=error_sum+error
        e_angle_sum = e_angle_sum+e_angle_target
        motorspeed=(Kp* error) +Kd*(error-error_prev)+Ki*(e_angle_sum)
        error_prev=error

        rightMotorSpeed = 30 + motorspeed
        leftMotorSpeed = 30 - motorspeed 
        ...
        file.write('\n'+'Kp='+str(Kp)+'|'+str(Kd)+'|'+str(Ki)+'\n')
        time.sleep(0.1)
        ser.write('2')#step_response
        time.sleep(0.1)
...
# pixels
dr = 1000 # mm, distance along the laser line to aim for
