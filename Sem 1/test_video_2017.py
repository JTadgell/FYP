...
tuning_mode=False
# Get command line inputs nicely
if(len(sys.argv)>1):
    try:
        opts, args = getopt.getopt(sys.argv[1:],"ht",["help","tune"])
        for opts, args in opts:
            if opts in ('-h','--help'):
                print('\n%s usage:'%sys.argv[0])
                print(" -h,--help:\tPrint this message")
                print(" -t --tune:\tStart the program with the option to tune the pids\n")
                sys.exit(0);
            elif opts in ('-t','--tune'):
                # setup pid tuning
                tuning_mode=True
                print("Starting in PID tuning mode")
    except getopt.GetoptError:
        print('Invalid argument, running normally')


# DEBUGGER = FALSE: RUN THE PROGRAM WITHOUT SHOWING IMAGE
debugger = True

# Initialise the Client IP Interface
try:
    host = "192.168.2.1"
    port = 13000
    addr = (host,port)
    UDPSock = socket(AF_INET, SOCK_DGRAM)
    print("UDP socket initialised")
except:
    print("Error: could not initialise UDP socket")
    sys.exit(1)
...
try:
    ...
    print("PSoC uart connection initialised")
except:
    print("Error: could not initialise PSoC uart connection")
    UDPSock.close()
    sys.exit(1)
    
# Initialise connection with the Arduino board
ser.write(bytes("<m600>","UTF-8"))
rec = ser.readline()
if(str(rec[0])=='49'):
    print("PSoC started main loop");
...
# pixels
dr = 500 # mm, distance along the laser line to aim for
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if dots == 2:
        ...
        if (frontdot[0] > 0) or (backdot[0] < 0):
            ...
            # Transmit the current angle and the target angle to the arduino
            ser.write(bytes('<'+str(int(e_angle))+'>', 'UTF-8'))

            # Send the x-position of the back dot to the Master
            tcpdata = str(backdot[0])
            tcpdata = bytes(tcpdata,'UTF-8')
            try:
                UDPSock.sendto(tcpdata,addr)
            except socket_error:
                print ("Send to Master Failed")
                break
    ...
    if debugger:
        ...
        cv2.putText(image, "X ->", (xzero+3,yzero-3),
                    cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1)
        cv2.putText(image, "\\/ Y", (xzero+3,yzero+15),
                    cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1)
    ...
    if tuning_mode == True:
       if key in ['p']:
           ...
           in_buf=str(key)
           leng=1
           print("Editing PIDs for [m]otor or [a]ngle, or [s]ave:{}".format(in_buf))
           while key not in ['\r','\n']:
               ...
               if leng==1:
                   ...
                   if key=='\b':
                       leng=0
                       break
                   #get 'm' or 'a' for motor or angle adjustment, s to save
                   elif key in 'msa':
                       in_buf+=str(key) 
                       leng=2
                       if key=='s':
                           ...
                           print("Choose the [p]roportional, [i]ntegral, or [d]ifferential gain:{}".format(in_buf))
               elif leng==2:
                   #check if backspace
                   if key=='\b':
                       leng=1
                   ...
                   #get either 'p','i', or 'd'
                       ...
                       in_buf+=str(key) 
                       leng=leng+1           
                       print("Enter new number:{}".format(in_buf))
               else:
                   #check if backspace
                   if key=='\b':
                       leng=leng-1
                       in_buf=in_buf[0:leng]
                       if(leng>2):
                           print("Enter new number:{}".format(in_buf))
                       elif(leng==2):
                           print("Choose the [p]roportional, [i]ntegral, or [d]ifferential gain:{}".format(in_buf))
                   #allow user to adjust number
                   elif str(key).isdigit() or key==ord('.'):
                       in_buf+=str(key); 
                       leng=leng+1
                       print("Enter new number:{}".format(in_buf))
           print("Length: {}, in_buf[3]: {}".format(leng,in_buf[3]))
           if( (in_buf[1]=='s' and leng == 2) or (leng>4 and in_buf[3].isdigit()) ):
               #can probably send data
               print("Sending: %s"%in_buf)
               ser.write(bytes(in_buf,"UTF-8"))
               rec = ser.readline()
               rec_str=''
               for a in rec:
                   rec_str.join(str(chr(a)))
               print(''.join([str(chr(a)) for a in rec]))
               time.sleep(3)
           print("PID editing finished")
    if key == 'q':
        break
    #debug slowdown
    time.sleep(1)
